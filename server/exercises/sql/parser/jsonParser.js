module.exports = async sourceCode => {
    return new Promise((resolve, reject) => {
      try {
          ...
          parsedDB.edges = parsedDB.tables.reduce((edges, table) => 
              [...edges, ...table.keyConstraints.filter(constraint => constraint.hasOwnProperty('foreign')).map(constraint => 
                ([{table: table.tableName, column: constraint.foreign}, {table: constraint.referencing.table, column: constraint.referencing.column}])
              )], [])
          resolve(parsedDB);
      } catch (err) {
        if (err) reject(err);
...
const parse = statement => statement.split('\n').reduce((table, line) => {
    ...
    const columnRegex2 = /^["\[ ]?((?!NOT|NULL|DELETE)\w{1,200})["\] ]\s?(\w{1,200})\s?(PRIMARY KEY)?/gmi;
    const foreignKeyRegex = /["\[]?([\w, ]*)["\]]?\) REFERENCES ["\`[]?(\w*)["`\]]?\s?\(["\[]?([\w, ]*)["\]]?/i;
    const primaryKeyRegex = /\(["\[]?([\w, ]*)["\]]?/g;

    const multipleKeys = string => string.split(',').map(column => column.replace(/["\(\[\] ]/g, ""));

    // table name
    if (tableNameRegex.test(line)) table.tableName = line.match(tableNameRegex)[1];
    // primary key
    else if (/(PRIMARY KEY)\s?\(.*\)/i.test(line) && primaryKeyRegex.test(line))
    ...
    // column name
    } else if (!/foreign key/i.test(line) && columnRegex1.test(line) || columnRegex2.test(line)) {
